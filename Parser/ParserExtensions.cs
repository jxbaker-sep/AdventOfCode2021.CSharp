...
public class ParseException(string? message) : ApplicationException(message)
{
}
...
public static class ParserExtensions
{
  ...
  public static T Parse<T>(this Parser<T> parser, string x) {
    ...
    throw new ParseException((m as ParseFailure<T>)!.Message);
  }

  public static bool TryParse<T>(this Parser<T> parser, string x, out T result) {
    var m = parser.Parse(x.ToCharArray(), 0);
    if (m is ParseSuccess<T> r) {
      result = r.Value;
      return true;
    }
    result = default!;
